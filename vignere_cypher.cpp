...
char shiftChar(char c, int kshift)
{
    ...
    {                // 65 to 90, uppercase letters
        ...
        if (c > 90)
        {            // last letter of alphabet, make it wrap around
...
string encryptVigenere(string plaintext, string keyword)
{
    ...
    { // reads in all plaintext chars
        if (n > keyword.size() - 1)
        { // keyword - 1 to not include last letter
            n = 0;
        }
        ...
        if (isalpha(c) == 0)
        {                                   // checks if c is an alphabetical char, 0 is false
